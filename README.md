...
Function of type `FlowFunction` have the responsibility to trigger the `resolve` callback and can be used to animate your pages. Those functions are called in `usePageFlow` to trigger the route change. 
`props` are the Vue Refs you want access for the animations. Those are the ones passed to the `usePageFlow`.
